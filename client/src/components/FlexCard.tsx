...
// Generate a deterministic avatar URL using DiceBear (no external assets required)
function avatarUrl(seed: string, size = 800) {
  const s = encodeURIComponent(seed || 'polyfield');
  return `https://api.dicebear.com/8.x/identicon/svg?seed=${s}&size=${size}`;
}
...
export function FlexCard({
  ...
  const characterUrl = avatarUrl(title || character || 'polyfield');
