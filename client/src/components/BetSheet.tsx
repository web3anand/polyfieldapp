...
export function BetSheet({ market, onClose }: BetSheetProps) {
  ...
  const selectedOrderBookData = orderBook || {
    yes: { bids: [], asks: [] },
    no: { bids: [], asks: [] },
  };
  ...
  const selectedOrderBook = betSide === 'yes' ? selectedOrderBookData.yes : selectedOrderBookData.no;
  ...
  const handlePlaceBet = async () => {
    try {
      ...
      if (!clob.isAuthenticated) {
        toast.error('Please configure authentication in Profile settings to place orders');
        return;
      }
      
      // Get token ID for the selected side (YES or NO)
      const tokenId = getTokenIdFromMarket(market, betSide);
      
      if (!tokenId) {
        toast.error('Token ID not available for this market. Please try again later.');
        return;
      }
  ...
  const setMaxAmount = () => {
    // Max amount will be determined by wallet balance
    setAmount('');
  };
        ...
        <div className="bg-[var(--bg-card)] rounded-t-3xl shadow-2xl max-h-[90vh] overflow-y-auto border-t border-[var(--border-color)]">
          ...
          <div className="p-6 space-y-4 pb-24">
            ...
            <div className="flex items-center gap-3">
              <ImageWithFallback
                src={market.imageUrl || `https://api.dicebear.com/8.x/identicon/svg?seed=${encodeURIComponent((market as any).slug || (market as any).question || 'market')}&size=100`}
                alt={market.title}
                    ...
                    {[25, 50, 75].map((pct) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          // Balance should be fetched from wallet
                          const currentAmount = parseFloat(amount) || 0;
                          setAmount((currentAmount * pct / 100).toFixed(2));
                        }}
            ...
            {orderType === 'limit' && (
              <>
                ...
                <div>
                  ...
                  {/* Matching Indicator */}
                  {orderType === 'limit' && selectedOrderBook.asks.length > 0 && (
                    <div className={`mb-2 px-3 py-1.5 rounded-lg ${
                      ...
                      <span className={`text-xs font-semibold ${action === 'buy' ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {selectedOrderBook.asks.length} order{selectedOrderBook.asks.length !== 1 ? 's' : ''} available
                      </span>
                    </div>
                  )}
