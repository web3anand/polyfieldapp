...
export function PortfolioPage() {
  const [activeView, setActiveView] = useState<'positions' | 'history' | 'orders' | 'holdings'>('positions');
                  ...
                  <div key={position.id} className="glass-card rounded-2xl p-4">
                    <div className="flex items-start gap-3">
                      <div className={`w-12 h-12 rounded-full overflow-hidden border border-[var(--border-color)] flex-shrink-0 ${
                        ...
                        <img 
                          src={(position as any).marketImage || `https://api.dicebear.com/8.x/identicon/svg?seed=${encodeURIComponent(((position as any).marketTitle || (position as any).market || 'market'))}&size=100`}
                  ...
                  <div key={trade.id} className="glass-card rounded-xl p-3">
                    <div className="flex items-start gap-3 mb-2">
                      <div className={`w-10 h-10 rounded-lg overflow-hidden border border-[var(--border-color)] bg-[var(--hover-bg)]`}>
                        <img 
                          src={(trade as any).marketImage || `https://api.dicebear.com/8.x/identicon/svg?seed=${encodeURIComponent(((trade as any).marketTitle || (trade as any).market || 'market'))}&size=100`}
                          alt={(trade as any).market || (trade as any).marketTitle || 'market'}
