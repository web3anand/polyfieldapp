...
export function ProfilePage() {
  ...
  // CLOB client for live user data (orders, holdings)
  ...
  const [totalHoldingsValue, setTotalHoldingsValue] = useState<number | null>(null);
  ...
  // Load live stats (best-effort). We derive basic metrics from holdings and orders.
  useEffect(() => {
    let mounted = true;
    async function loadStats() {
      try {
        const [orders, holdings] = await Promise.all([
          clob.getUserOrders(),
          clob.getUserHoldings(),
        ]);

        if (!mounted) return;

        // Total bets = number of orders
        setTotalBets(Array.isArray(orders) ? orders.length : 0);

        // Total volume: sum of order.size * price where available
        const volume = (orders || []).reduce((acc: number, o: any) => {
          const size = parseFloat(o.size || '0') || 0;
          const price = parseFloat(o.price || '0') || 0;
          return acc + size * price;
        }, 0);
        setTotalVolume(isFinite(volume) ? volume : 0);

        // Positions/holdings count
        setPositionsCount(Array.isArray(holdings) ? holdings.length : 0);

        // Total holdings value: best-effort using token balances only (no price lookup)
        const holdingsValue = (holdings || []).reduce((acc: number, h: any) => {
          const bal = parseFloat(h.balance || '0') || 0;
          // Without reliable price data here, we report token quantity sum as a proxy
          return acc + bal;
        }, 0);
        setTotalHoldingsValue(isFinite(holdingsValue) ? holdingsValue : 0);
      } catch (e) {
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 pb-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
          <div className="glass-card rounded-2xl p-4">
            ...
            <div className="text-xl font-semibold text-[var(--text-primary)]">{totalBets === null ? '—' : totalBets}</div>
          </div>
          <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-4 text-white shadow-lg">
            ...
            <div className="text-xl font-bold">{/* Win rate not currently available from public APIs */}—</div>
          </div>
          <div className="glass-card rounded-2xl p-4">
            <div className="text-xs text-[var(--text-secondary)] uppercase tracking-wider font-medium mb-1">Total Volume</div>
            <div className="text-xl font-semibold text-[var(--text-primary)]">{totalVolume === null ? '—' : `$${totalVolume.toFixed(2)}`}</div>
          </div>
        </div>
