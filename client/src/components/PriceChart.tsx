...
// Generate chart data from current prices
// In production, this should fetch historical price data from the API
const generateChartData = (yesPrice: number, noPrice: number) => {
  // For now, show current prices as a single data point
  // TODO: Fetch historical price data from Polymarket API
  const now = new Date();
  return [{
...
export function PriceChart({ market, betSide }: PriceChartProps) {
  const data = generateChartData(market.yesPrice, market.noPrice);
