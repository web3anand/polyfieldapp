 ...
 * 
 * NO CACHING - Markets fetched once on mount, WebSocket provides live price updates
 */
export async function getMarketsViaProxy(
  ...
  try {
    ...
    } else if (env.isProduction && !(window as any).__polymarket_proxy_attempt) {
      console.info('🔄 Fetching markets via Vercel serverless function...');
      (window as any).__polymarket_proxy_attempt = true;
    }
    ...
    const markets: Market[] = rawMarkets
      ...
      .map((market: any) => {
        ...
        // Polymarket API may include token IDs in various formats
        ...
        if (Array.isArray(market.outcomes)) {
          ...
          const noOutcome = market.outcomes.find((o: any) => 
        ...
        // If we have conditionId but no token IDs, we'll need to derive them later
        // For now, store conditionId for future token ID resolution
